...
function create() {
    ...
    const socket = Socket.getSocket();
    if (socket) {
        socket.on('player_movement', (moveData) => this.handlePlayerMovement(moveData));
        socket.on('weapon_spawn', (weaponData) => this.handleWeaponSpawn(weaponData));
        socket.on('powerup_spawn', (powerupData) => this.handlePowerupSpawn(powerupData));
...
function startGame(gameData, socketId) {
    ...
    if (window.game?.scene?.scenes[0]?.isActive) {
        ...
        spawnAllPlayers.call(window.game.scene.scenes[0], gameData.players, socketId);
    }
...
function spawnAllPlayers(playerAssignments, socketId) {
    ...
    playerAssignments.forEach(playerData => {
        ...
        const newPlayer = PlayerManager.createPlayer(this, {
            ...playerData,
            x: spawnPoint.x,
            y: spawnPoint.y
        }, spawnPoint);
        ...
        const nameLabel = this.add.text(spawnPoint.x, spawnPoint.y - 20,
            playerData.username, {
            fontSize: '14px',
            fill: '#fff'
        }).setOrigin(0.5);
