...
function startItemSpawning(roomId, io, game) {
    ...
    const powerupTimer = setInterval(() => {
        ...
        // Randomly select a powerup type from configured options (HEALTH, ATTACK, SPEED)
        const powerupType = SPAWN_CONFIG.POWERUPS.types[
            Math.floor(Math.random() * SPAWN_CONFIG.POWERUPS.types.length)
        ];

        // Create powerup data object with specific configurations
        const powerupData = {
            roomId,
            type: powerupType,
            powerupConfig: {
                name: powerupType.toLowerCase(),
                // Health powerup restores 20 HP, others have no direct healing
                effect: powerupType === 'HEALTH' ? 20 : 0,
                // Stat multipliers: Attack=2x, Speed=1.5x, Health=1x (no multiplier)
                multiplier: powerupType === 'ATTACK' ? 2 : powerupType === 'SPEED' ? 1.5 : 1,
                // Effect duration: Attack=10s, Speed=8s, Health=instant (0ms)
